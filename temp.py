# Building a Data Cleansing and Transformation solution using Machine Learning
import pandas as pd
import numpy as np
  ...
  ">
    ...
    ">
      <div style="
        ...
        width: 40px;
        height: 40px;
        background: radial-gradient(circle at center, #E91E63, #673AB7, #00BCD4, #4CAF50);
        border-radius: 50%;
        animation: pulse-and-move 3s ease-in-out infinite;
      "></div>
      <div style="
        position: absolute;
        width: 40px;
        height: 40px;
        background: radial-gradient(circle at center, #00BCD4, #4CAF50, #E91E63, #673AB7);
        border-radius: 50%;
        animation: pulse-and-move 3s 1s ease-in-out infinite;
      "></div>
      <div style="
        position: absolute;
        font-size: 60px;
      ...
      <div style="
        position: absolute;
        font-size: 60px;
        animation: emoji-bounce 2s 0.5s ease-in-out infinite;
      ">🔄</div>
      <div style="
        position: absolute;
        font-size: 60px;
        animation: emoji-bounce 2s 1s ease-in-out infinite;
      ">🤖</div>
    </div>
  </div>
  <div style="
...
def save_data_clean_file(df, output_file='data_cleaned.csv'):
    if not isinstance(df, pd.DataFrame):
        st.error("The provided input is not a valid DataFrame.")
        logging.error("The provided input is not a valid DataFrame.")
        return None
    ...
    try:
        ...
        st.error(f"Permission denied: Unable to save file {output_file}.")
        logging.error(f"Permission denied: Unable to save file {output_file}.")
        raise PermissionError(f"Permission denied: Unable to save file {output_file}.")
    except Exception as e:
        st.error(f"An error occurred while saving the file {output_file}: {e}")
        logging.error(f"An error occurred while saving the file {output_file}: {e}")
        raise RuntimeError(f"An error occurred while saving the file {output_file}: {e}")
    ...
    for col in columns_by_type.get('nominal', []):
        if col in df_trans.columns:
            try:
                ...
                new_cols = [c for c in dftrans.columns if c.startswith(col + '')]
